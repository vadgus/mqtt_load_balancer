...
def _run_service(service_label: str, index: int) -> Process:

    arguments = [f'--index={index}']
    cmd = [sys.executable, 'worker.py'] + arguments

    print('[SERVICE START] %s %s' % (service_label, json.dumps(arguments)))
...
def main(config: ConfigParser):
    ...
    for section, section_config in config.items():
        ...
        process_data = ProcessData(process=process, pid=process.pid)
